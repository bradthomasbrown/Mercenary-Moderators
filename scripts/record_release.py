...
if api("repos/" + repository)["private"] is not True:
    raise SystemExit("Private-first publication requires a private repository.")
...
record = {
    ...
    "status": "published-private-prerelease" if success else "workflow-failed-check-actions",
    "tag": tag, "sourceCommit": subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip(),
    "repositoryPrivate": True, "installer": release["filename"], "sha256": release["sha256"],
...
if existing:
    ...
    if previous.get("status") == "published-private-prerelease":
